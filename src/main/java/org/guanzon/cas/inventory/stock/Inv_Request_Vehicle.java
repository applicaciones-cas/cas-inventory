 ...
 * @author Unclejo
 */
public class Inv_Request_Vehicle implements RequestController {
    ...
    public Inv_Request_Vehicle(GRider foGRider, boolean fbWthParent) {
        poGRider = foGRider;
...
//                + SQLUtil.toSQL(fsValue + "%") + " AND " + lsCondition + " AND f.sCategCd1 = '0001' AND f.sCategCd2 != '0007' GROUP BY a.sTransNox ASC");
    ...
    public JSONObject loadAllInventoryMinimumLevel(){
        ...
        try {
            ...
            lsSQL = MiscUtil.addCondition(lsSQL, "a.nQtyOnHnd < a.nMinLevel AND  b.sCategCd1 = '0003' ");
            ResultSet loRS = poGRider.executeQuery(lsSQL);
            System.out.println(" " + lsSQL);
            poModelDetailOthers =  new ArrayList<>();
            while (loRS.next()) {
    ...
    private JSONObject SaveDetail(){
        ...
        if (getItemCount() >= 1) {
            ...
            for (int lnCtr = getItemCount() - 1; lnCtr >= 0; lnCtr--) {
                    ...
                    if (poModelDetail.get(lnCtr).getQuantity().equals(0)|| poModelDetail.get(lnCtr).getQuantity() == null) {
                        // Remove the empty record
                        poModelDetail.remove(lnCtr);
                    }
                }

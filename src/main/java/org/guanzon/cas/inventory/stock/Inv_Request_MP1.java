 ...
 * @author Unclejo
 */
public class Inv_Request_MP1 implements RequestController {
    ...
    public Inv_Request_MP1(GRider foGRider, boolean fbWthParent) {
        poGRider = foGRider;
...
//                    if (poJSON != null) {
            ...
            case 26://brand
                Brand loBrand = new Brand(poGRider, false);
                loBrand.setRecordStatus(psTranStatus);
                if (!System.getProperty("store.inventory.industry").isEmpty()){
                    lsCondition = " a.sCategrCd = '0002' ";
                }
                poJSON = loBrand.searchRecord(fsValue, fbByCode, lsCondition);
                System.out.println("poJSON = " + poJSON);
                if (poJSON != null) {
...
//                loModel.setRecordStatus(psTranStatus);
                ...
                lsSQL = getSQLModelInventory();
                 System.out.println("search LSSQL == " + lsSQL);
                if(!getDetailModel().get(fnRow).getBrandName().isEmpty()){
                    lsSQL = MiscUtil.addCondition(lsSQL, " f.sDescript = " + SQLUtil.toSQL(getDetailModel().get(fnRow).getBrandName()));
                }
                lsSQL = MiscUtil.addCondition(lsSQL, "g.sDescript LIKE " + SQLUtil.toSQL("%" + fsValue + "%"));
                lsSQL = MiscUtil.addCondition(lsSQL, "a.sCategCd2 = '0008'");
                lsSQL = MiscUtil.addCondition(lsSQL, "a.sCategCd3 = '0001'");
                System.out.println("search Model === " + lsSQL);
//                poJSON = loModel.searchRecord(fsValue, fbByCode, lsCondition);
//                
//                System.out.println("poJSON = " + poJSON);
//                if (poJSON != null) {
...
//                    System.out.println(loModel.getModel().getModelDescription());
//                } else {
//                    poJSON = new JSONObject();
//                    poJSON.put("result", "error");
//                    poJSON.put("message", "No record found.");
...
//                }
                poJSON = ShowDialogFX.Search(poGRider,
                    lsSQL,
                    fsValue,
                    "Stock ID»Barcode»Name»Brand»Model",
                    "sStockIDx»sBarCodex»sDescript»sDescript»sDescript",
                    "a.sStockIDx»a.sBarCodex»a.sDescript»a.xBrandNme»a.xModelNme",
                    fbByCode ? 1: 2);

                if (poJSON != null) {
                        setDetail(fnRow, 3, (String) poJSON.get("sStockIDx"));
                        setDetail(fnRow, "xBarCodex", (String) poJSON.get("sBarCodex"));
                        setDetail(fnRow, "xDescript", (String) poJSON.get("sDescript"));
                        setDetail(fnRow, "xCategr01", (String) poJSON.get("xCategNm1"));
                        setDetail(fnRow, "xCategr02", (String) poJSON.get("xCategNm2"));
                        setDetail(fnRow, "xInvTypNm", (String) poJSON.get("xInvTypNm"));
                        setDetail(fnRow, "xBrandNme", (String) poJSON.get("xBrandNme"));
                        setDetail(fnRow, "xModelNme", (String) poJSON.get("xModelNme"));
                        setDetail(fnRow, "xModelCde", poJSON.get("xModelCde"));
                        setDetail(fnRow, "xColorNme", poJSON.get("xColorNme"));
                        setDetail(fnRow, "xMeasurNm", poJSON.get("xMeasurNm"));
                        
                        
                        setDetail(fnRow, "cClassify", poJSON.get("cClassify"));
                        setDetail(fnRow, "nQtyOnHnd", poJSON.get("nQtyOnHnd"));
                        setDetail(fnRow, "nResvOrdr", poJSON.get("nResvOrdr"));
                        setDetail(fnRow, "nBackOrdr", poJSON.get("nBackOrdr"));
                        setDetail(fnRow, "nAvgMonSl", poJSON.get("nAvgMonSl"));
                        setDetail(fnRow, "nMaxLevel", poJSON.get("nMaxLevel"));
                        
                        setDetail(fnRow, "xVrntName", poJSON.get("xVrntName"));
                        setDetail(fnRow, "sVrntDesc", poJSON.get("sVrntDesc"));
                        setDetail(fnRow, "sSeriesNm", poJSON.get("sSeriesNm"));
...
//        AddModelDetail();
        ...
        poJSON = poModelDetailOthers.get(getItemCount() - 1).setTransactionNumber(poModelMaster.getTransactionNumber());
...
//        AddModelDetail();
    ...
    @Override
    public JSONObject searchTransaction(String fsCol, String fsValue, boolean fbByCode) {
        String lsCondition = "";
        String lsFilter = "";
        ...
        System.out.println("Search lsCondition == " + lsCondition);
        String lsSQL = MiscUtil.addCondition(getSQL(), " a.sTransNox LIKE "
                + SQLUtil.toSQL(fsValue + "%") + " AND f.sCategCd1 = '0002' AND f.sCategCd2 = '0008' AND f.sCategCd3 = '0001' AND " + 
        ...
        System.out.println("Search Trans == " + lsSQL);
//        String lsSQL = MiscUtil.addCondition(getSQL(), " a.sTransNox LIKE "
//                + SQLUtil.toSQL(fsValue + "%") + " AND " + lsCondition + " AND f.sCategCd1 = '0001' AND f.sCategCd2 != '0007' GROUP BY a.sTransNox ASC");
    ...
    public JSONObject loadAllInventoryMinimumLevel(){
        ...
        try {
            ...
            lsSQL = MiscUtil.addCondition(lsSQL, "a.sBranchCd = '" + poGRider.getBranchCode() + "' AND  a.nQtyOnHnd <= a.nMinLevel AND  b.sCategCd1 = '0002' AND b.sCategCd2 = '0008' and  b.sCategCd3 = '0001'");
            
            ResultSet loRS = poGRider.executeQuery(lsSQL);
            System.out.println("loadAllInventoryMinimumLevel " + lsSQL);
            poModelDetailOthers =  new ArrayList<>();
            while (loRS.next()) {
                poModelDetailOthers.add(new Model_Inv_Stock_Request_Detail(poGRider));
                ...
                poModelDetailOthers.get(poModelDetailOthers.size() - 1).setSeriesName(loRS.getString("sSeriesNm"));
                poModelDetailOthers.get(poModelDetailOthers.size() - 1).setVariantName(loRS.getString("xVrntName"));
                poModelDetailOthers.get(poModelDetailOthers.size() - 1).setVariantDescription(loRS.getString("sVrntDesc"));
        ...
        if (getItemCount() >= 1) {
            ...
            for (int lnCtr = getItemCount() - 1; lnCtr >= 0; lnCtr--) {
                    ...
                    if (poModelDetail.get(lnCtr).getStockID().isEmpty() || poModelDetail.get(lnCtr).getBarcode().isEmpty() ||
                            poModelDetail.get(lnCtr).getQuantity().equals(0)|| poModelDetail.get(lnCtr).getQuantity() == null) {
...
//            poJSON = SaveDetail();
    ...
    private String getSQL(){
	    ...
	    "  LEFT JOIN Category_Level3 g " +
            "   on f.sCategCd3 = g.sCategrCd ";
    }
    ...
    private String getSQL_Detail(){
        return "SELECT " +
                ...
                " k.sVrntName xVrntName, " +
                " k.sDescript sVrntDesc, " +
                " j.sDescript sSeriesNm " +
                "FROM  Inv_Master a " +
                ...
                " LEFT JOIN Model_Series j ON f.sSeriesID = j.sSeriesID" +
                " LEFT JOIN Model_Variant k ON f.sVrntIDxx = k.sVrntIDxx";
    }
    
    public String getSQLModelInventory(){
        String lsSQL = "SELECT" +
                            "  a.sStockIDx" +
                            ", a.sBarCodex" +
                            ", a.sDescript" +
                            ...
                            ", l.cClassify cClassify" +
                            ", l.nQtyOnHnd" +
                            ", l.nResvOrdr" +
                            ", l.nBackOrdr" +
                            ", l.nAvgMonSl" +
                            ", l.nMaxLevel" +
                            ", ggg.sVrntName xVrntName" +
                            ", ggg.sDescript sVrntDesc" +
                            ", gg.sDescript sSeriesNm" +
                        " FROM Inventory a"+ 
                            ...
                            " LEFT JOIN Model_Series gg ON g.sSeriesID = gg.sSeriesID" +
                            " LEFT JOIN Model_Variant ggg ON g.sVrntIDxx = ggg.sVrntIDxx" +
                            ...
                            " LEFT JOIN Inv_Master l ON a.sStockIDx = l.sStockIDx" + 
                            " LEFT JOIN Warehouse m ON l.sWhouseID = m.sWhouseID" +
                            " LEFT JOIN Inv_Location n ON l.sLocatnID = n.sLocatnID" +
                            " LEFT JOIN Section o ON o.sSectnIDx = n.sSectnIDx";

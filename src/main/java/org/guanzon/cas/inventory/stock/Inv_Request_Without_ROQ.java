 ...
 * @author Unclejo
 */
public class Inv_Request_Without_ROQ implements RequestController {

    GRider poGRider;
    boolean pbWthParent;
    int pnEditMode;
    String psTranStatus;
    String psCategory_Lvl2 = "";
    ...
    public Inv_Request_Without_ROQ(GRider foGRider, boolean fbWthParent) {
        poGRider = foGRider;
    ...
    public JSONObject searchDetail(int fnRow, int fnCol, String fsValue, boolean fbByCode) {
        ...
        switch (fnCol) {
            ...
            case 3: //sBarCodex
                ...
                if(!psCategory_Lvl2.isEmpty()){
                    poJSON = loInventory.searchRecordWithContition(fsValue,  psCategory_Lvl2, fbByCode);

                }else{
                    poJSON = loInventory.searchRecord(fsValue, fbByCode);
...
//                    if (poJSON != null) {
    ...
    public JSONObject searchTransaction(String fsColumn, String fsValue, boolean fbByCode) {
        ...
        if(type == RequestControllerFactory.RequestType.MC){
            lsSQL = MiscUtil.addCondition(lsSQL, "f.sCategCd2 != '0007'");
        }
        lsSQL = lsSQL + " GROUP BY a.sTransNox ASC";
    ...
    */
    ...
    public JSONObject loadAllInventoryMinimumLevel(){
        ...
        try {
            ...
            if(!psCategory_Lvl2.isEmpty()){
                lsSQL = MiscUtil.addCondition(lsSQL, "b." + psCategory_Lvl2);
            }
            ResultSet loRS = poGRider.executeQuery(lsSQL);
            System.out.println("\n" + lsSQL);
            poModelDetail =  new ArrayList<>();
